...
// Fetch Orders 
$order_query = "select * from orders where track_status = 'Out of Delivery'";
...
if (isset($_REQUEST['update_track'])) {
    ...
    if ($track_change_value == 'Delivered') {
        $update_track_query = "update orders set track_status='Delivered' where order_id = '$order_id'";
        mysqli_query($db, $update_track_query);
        header("location: outdelivery.php");
    } else {
        echo "<script>
...
<body>
    ...
    <div class="orders_tab">
        ...
        <button class="co_tab" onclick="location.href='outdelivery.php'">Out Of Delivers</button>
    ...
    if ($count_orders > 0) {
        ...
        <div class="no_orders">
            <h2>No Out For Delivery Orders Found</h2>
        </div>
    <?php } ?>
