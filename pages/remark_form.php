...
if (isset($_REQUEST['update_form'])) {
    ...
    $sql = "select * from train_forms where train_form_id = '$id'";
...
$getchecked = mysqli_fetch_array(mysqli_query($db, "select * from train_forms where train_form_id = '$id'"));
...
if (isset($_REQUEST['update_application'])) {
    ...
    $sql = "UPDATE train_forms SET 
                ...
                signature='$sign', 
                status='pending' 
            WHERE train_form_id = '$id'";

    mysqli_query($db, $sql);

    // Remove related notifications
    $notifi_form_id = $vk['train_form_id'];
...
// Read Dropdown Data
$user_email = $user;
$user_query = "select * from train_forms where email = '$user_email'";
